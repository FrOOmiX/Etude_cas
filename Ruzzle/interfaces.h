...
typedef struct{
    ...
    int coord[17][2];
    SDL_Surface *title, *background, *faces, *trophy, *letter, *timer, *scoreDisplay;
    SDL_Rect logoRuzzlePosition, backgroundPosition, trophyPosition, clip[5], clipClic[5], letterPosition, pos, timerPosition, scorePos;
...
extern GridWindow* grid_window_create(int *pScore, Cell gride[N][N]);
...
extern int grid_window_draw_on_clic(GridWindow* grid, SDL_Surface* screen, SDL_Event event, Trie *t, Trie *tGrid, Cell gride[N][N], int coord[17][2], int *cpt, int *pScore);
extern int onClic(GridWindow* grid,int x,int y, int *pointeurSurA, int *pointeurSurB,int *pointeurSurR,int *pointeurSurC);
extern void bonusOrange(GridWindow* grid, SDL_Surface* screen, int *pointeurSurA, int *pointeurSurB);
